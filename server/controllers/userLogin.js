...
exports.loginUser = async (req, res) => {
  try {
    ...
    if (role === "teacher") {
      ...
      const teacherLogin = client.query(query, (err, result) => {
        if (result.rows.length === 0) {
          res.json({ noUser: "User not found" });
          return;
        }
    ...
    if (role === "student") {
      ...
      const studentLogin = client.query(query, (err, result) => {
        if (result.rows.length === 0) {
          res.json({ noUser: "User not found" });
          return;
        }
...
exports.registerUser = async (req, res) => {
  try {
    ...
    if (role === "student") {
      ...
      let query = `insert into students(stuid, cid, name, email, password, role) values('${stuid}', '${cid}', '${name}', '${email}', '${password}', '${role}')`;
